    ...
    for(hist_file of Object.keys(file_match_up)){
        ...
        try{
            if(parseInt(file_match_up[hist_file][1].split('_')[2]) + 1 == parseInt(hist_file.split('_')[2])){
                ...
                let morning_json = await parse_file_text(forecast_morning, hist_file)
                let evening_json = await parse_file_text(forecast_evening, hist_file)
                let hist_json = await parse_file_text(hist_file, hist_file)
                
                console.log(get_real_feel_temp_stat_test(morning_json, evening_json, hist_json, 4))
                console.log('end')
            }
            else{
...
/*helper function that takes a text file that gets parsed into a JS object and retrieves data from a certain date of a forecast*/
...
async function parse_file_text(text_data, history_file){
    let hour = ''
    let date = 0
    let file_date = history_file.split('_')
    let folder_path = ''
    if(text_data != hist_file){
        if(text_data == file_match_up[hist_file][0]){
            hour = '8'
        }
        else{
            hour = '20'
...
//test_stats_setup()
async function test_stats_setup(){
    let hist_file = '/2020_7_10_9_24hr_hist.txt'
    let morning_data = await new Promise((resolve, reject) =>{
        fs.readFile(path.join(__dirname, forecasts_folder_path  + '/2020_7_8_20_12hr_forecast.txt'), "utf8", (err, file_text) => {
            if(err) {
                reject(err)
                return;
            }
            ...
            file_date = file_date[0].substring(1,file_date[0].length) + '_' + file_date[1] + '_' + (parseInt(file_date[2]) - 1) + '_' + '8'
            parsed_json_forecast_am = parsed_json_forecast_am[file_date]
            resolve(parsed_json_forecast_am)
        });
    })
    let evening_data = await new Promise((resolve, reject) =>{
        fs.readFile(path.join(__dirname, forecasts_folder_path  +'/2020_7_9_20_12hr_forecast.txt'), "utf8", (err, file_text) => {
            if(err) {
                reject(err)
                return;
            }
            ...
            let file_date = hist_file.split('_')
            file_date = file_date[0].substring(1,file_date[0].length) + '_' + file_date[1] + '_' + (parseInt(file_date[2]) - 1) + '_' + '20'
            parsed_json_forecast_pm = parsed_json_forecast_pm[file_date]
            resolve(parsed_json_forecast_pm)
        });
    })
    let hist_data = await new Promise((resolve, reject) =>{
        fs.readFile(path.join(__dirname, historical_data_folder_path  +'/' + hist_file), "utf8", (err, file_text) => {
            ...
            let file_date = hist_file.split('_')
            file_date = file_date[0].substring(1,file_date[0].length) + '_' + file_date[1] + '_' + file_date[2] + '_' + '9'
    ...
    console.log(get_real_feel_temp_stat_test(morning_data, evening_data, hist_data, 3))
...
function get_real_feel_temp_stat(morning, evening, hist, precision){
    ...
    create_real_feel_dic(morning, real_feel_forecast)
    create_real_feel_dic(evening, real_feel_forecast)
    create_real_feel_dic(hist, real_feel_hist)
    function create_real_feel_dic(json_data, real_feel_dic){
        for(state of Object.keys(json_data)){
            if(json_data != evening){
                real_feel_dic[state] = {}
            }
...
function intitiate_stat_retrieval(morning, evening, hist){
